...
module.exports.performAddMovieToMember = async function (req, res, next) {
  try {
    let { data: subscription } = await axios.get(
      `http://subscriptions-rest-api.herokuapp.com/api/subscribers/${req.params.id}`
    )
    ...
    let { data: movie } = await axios.get(
      `http://subscriptions-rest-api.herokuapp.com/api/movies/${req.body.movieId}`
    )
    await axios.put(
      `http://subscriptions-rest-api.herokuapp.com/api/subscribers/${req.params.id}`,
      subscription,
...
module.exports.displayEditMember = async function (req, res, next) {
  try {
    let { data: editedMember } = await axios.get(
...
module.exports.performDeleteMember = async function (req, res, next) {
  try {
    ...
    let deleteSubscriber = subscribers.find(
      (subscriber) => subscriber.memberId === req.params.id
    )
    await axios.delete(
      `http://subscriptions-rest-api.herokuapp.com/api/subscribers/${deleteSubscriber._id}`
    )
